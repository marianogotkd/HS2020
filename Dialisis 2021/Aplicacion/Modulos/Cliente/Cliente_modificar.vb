...
Public Class Cliente_modificar
    ...
    Dim ds_paciente As DataSet
    Public Sub Obtener_Pacientes()
        If RadioButton_activo.Checked = True Then
            ds_paciente = DApaciente.Paciente_obtener_activos()
            Cliente_ds.Tables("Paciente1").Rows.Clear()
            If ds_paciente.Tables(0).Rows.Count <> 0 Then
                Cliente_ds.Tables("Paciente1").Merge(ds_paciente.Tables(0))
            End If
        Else
            If RadioButton_inactivo.Checked = True Then
                ds_paciente = DApaciente.Paciente_obtener_inactivos
                Cliente_ds.Tables("Paciente1").Rows.Clear()
                If ds_paciente.Tables(0).Rows.Count <> 0 Then
                    Cliente_ds.Tables("Paciente1").Merge(ds_paciente.Tables(0))
                End If
    ...
    Private Sub BO_cliente_modificar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BO_cliente_modificar.Click
        If DG_clientes.Rows.Count <> 0 Then
            ...
            Else
                MsgBox("Seleccione un paciente", MsgBoxStyle.Information)
            End If
        Else
            MsgBox("Seleccione un paciente", MsgBoxStyle.Information)
        End If
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        If DG_clientes.Rows.Count <> 0 Then
            Dim productoselec As String = DG_clientes.SelectedCells(0).Value
            If productoselec <> "" Then
                Informe_seleccionar.TextBox2.Text = DG_clientes.CurrentRow.Cells("PACdniDataGridViewTextBoxColumn").Value
                Informe_seleccionar.TextBox1.Text = DG_clientes.CurrentRow.Cells("PacienteDataGridViewTextBoxColumn").Value
                'Informe_seleccionar.cliente_id = CInt(productoselec)
                Informe_seleccionar.paciente_id = CInt(productoselec)
                Informe_seleccionar.Show()
            Else
                MsgBox("Seleccione un paciente", MsgBoxStyle.Information)
            End If
        Else
            MsgBox("Seleccione un paciente", MsgBoxStyle.Information)
        End If
        'le mando info de la fila seleccionada
    End Sub
    ...
    Private Sub RadioButton_inactivo_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles RadioButton_inactivo.CheckedChanged
        Obtener_Pacientes()
    End Sub

    Private Sub Cliente_modificar_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        msj_esperar_sesiones.procedencia = "Cliente_modificar_load"
